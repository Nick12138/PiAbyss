 ...
 *
 * Env:
 *   PIABYSS_SPIKE_NONCE  — required random nonce for this run
 *   PIABYSS_SPIKE_MARKER — absolute path for the marker file
 */
...
export default function spikeExtension(pi: ExtensionAPI) {
  pi.on("session_start", async () => {
    const nonce = process.env.PIABYSS_SPIKE_NONCE;
    const marker = process.env.PIABYSS_SPIKE_MARKER;
    if (!nonce || !marker) {
      throw new Error("spike-extension: missing PIABYSS_SPIKE_NONCE or PIABYSS_SPIKE_MARKER");
    }
