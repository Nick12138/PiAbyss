...
- [x] Recover the original Git history; do not create a provenance-free replacement baseline. (Resolved 2026-07-18: confirmed with the owner that no prior repository ever existed — the round-1 assumption of lost history was wrong. A fresh baseline was created intentionally and pushed to github.com/Skitre/PiAbyss.)
...
- [x] E2E window-attach waited for exact text "PiAbyss", which only renders in the unmounted `TitleBar.tsx` or with Settings open — timing-dependent. Anchor changed to the always-rendered "Pi Agent" sidebar brand. (commit ff30f0c)
